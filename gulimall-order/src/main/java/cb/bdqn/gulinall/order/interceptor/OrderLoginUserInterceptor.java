package cb.bdqn.gulinall.order.interceptor;
...
public class OrderLoginUserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        boolean match = pathMatcher.match("/order/order/order/state/**", requestURI);
        boolean match1 = pathMatcher.match("/pay/notify", requestURI);
        if (match | match1) {
            return true;
        }
